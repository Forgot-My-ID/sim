...
SERVER_BUSY = 1       # Server is serving a customer
SERVER_IDLE = 0       # Server is available
...
# Statistics tracking
queue_area = 0.0  # For average queue length calculation
...
def initialize():
    ...
    # Schedule first arrival, set departure to far future
...
def process_arrival():
    ...
    if server_state == SERVER_BUSY:
        ...
        # Server idle, begin service immediately
        wait_time = 0.0
...
def process_departure():
    ...
    if queue_length == 0:
        # No more customers, server goes idle
        server_state = SERVER_IDLE
        next_event_times[2] = 1.0e+30
    else:
